...
import ZZZSidebar from '../../components/ZZZSidebar';
import { zzzWishImportAPI } from '../../../APIs/wishImportAPI';

const ImportWish = ({ userID }) => {
  const [isCopied, setIsCopied] = useState(false);
  const [isButtonDisabled, setButtonDisabled] = useState(false);
  const navigate = useNavigate();
  const generatedLink = `Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; iex "&{$(irm https://raw.githubusercontent.com/NousIndex/draw_import/main/zzz_link.ps1)} global"`;
  ...
  async function runWishImportAPI(inputValue) {
    try {
      ...
      const response = await zzzWishImportAPI(inputValue, userID);
      let responseMessage = '';
  ...
  return (
    <div className="genshin-import-container">
      ...
      <div className="import-wish-container">
        <h2 className="genshin-import-text-title">Steps to Import Searchs:</h2>
        <ol className="genshin-import-instructions-container">
          <li className="genshin-import-text">
            Open Zenless Zone Zero on your PC.
          </li>
          <li className="genshin-import-text">
            Open the search history in the game and wait for it to load.
          </li>
          <li className="genshin-import-text">
