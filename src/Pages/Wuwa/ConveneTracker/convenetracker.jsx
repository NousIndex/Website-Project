...
import banner3 from '../../../assets/Icons/wuwa-weapon.webp';
import banner4 from '../../../assets/Icons/wuwa-standard.webp';
...
import ItemTable from './convenerecords.jsx';
...
function WishTracker({ userID }) {
  ...
  async function updateWatchList() {
    if (watchListOriginal !== watchList) {
      ...
      const url = `${API_URL}api/draw-watchlist?game=wuwa&command=update`;
  ...
  async function getWatchList() {
    try {
      const response = await fetch(
        `${API_URL}api/draw-watchlist?game=wuwa&command=get&userGameId=${userGameId}`
      );
      ...
      } else if (data.Wuwa_Watch === null) {
        setWatchList([]);
        setWatchListOriginal([]);
      } else {
        console.log(data);
        setWatchList(JSON.parse(data.Wuwa_Watch));
        setWatchListOriginal(JSON.parse(data.Wuwa_Watch));
      }
    } catch (error) {
      console.error('Error fetching API usage data:', error);
    }
  ...
  async function fetchData(userGameId) {
    try {
      const response = await fetch(
        `${API_URL}api/draw-history?game=wuwa&userGameId=${userGameId}`
      );
      const data = await response.json();
      setWishAPIData(data);
  ...
  useEffect(() => {
    async function fetchData2() {
      try {
        const response = await fetch(`${API_URL}api/draw-icons?game=wuwa`);
        const data = await response.json();
        setItemIcons(data);
      } catch (error) {
        console.error('Error fetching API usage data:', error);
  ...
  const imageButtonsArray = [
    ...
    },
    {
      ...
      onClick: () => {
        setBannerFilter('featured resonator');
        handleFilter('Featured Resonator Convene');
      },
    },
    {
      imageUrl: banner3,
      ...
      onClick: () => {
        setBannerFilter('featured weapon');
        handleFilter('Featured Weapon Convene');
      },
    },
    {
      imageUrl: banner4,
      ...
      onClick: () => {
        setBannerFilter('standard resonator');
        handleFilter('Standard Resonator Convene');
      },
    },
    {
      imageUrl: banner4,
      ...
      onClick: () => {
        setBannerFilter('standard weapon');
        handleFilter('Standard Weapon Convene');
      },
    },
  ];
  ...
  return (
    <div className="wishpage-container">
      ...
      <WuwaSidebar activeTab={'Convene Tracker'} />
      ...
      <div className="content">
        <h1 className="wishpage-main-title">
          Convene Tracker
          <a
            href={routePaths.WUWA_WISH_TRACKER_IMPORT_PATH}
