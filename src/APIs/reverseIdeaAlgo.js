async function findBestCombination(gridWidth, gridHeight, shapes) {
  console.time('computationTime'); // Start measuring computation time
    ...
    for (const shape of shapes) {
      for (
        ...
        if (shape.amount > 0 && canPlaceShape(shape, x, y, orientation)) {
          ...
          if (
            !memoizationCache.has(newState) ||
            memoizationCache.get(newState) < currentValue
          ) {
            // Check memoization cache for this state
            memoizationCache.set(newState, currentValue); // Store the current value
...
export async function findBestCombinationAPI(gridWidth, gridHeight, shapes) {
  const { bestValue, bestCombination } = findBestCombination(
    gridWidth,
    gridHeight,
    shapes
  );
  ...
  const grid = createGridFromArray(bestCombination, gridWidth, gridHeight);
